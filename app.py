...
def page_main_dashboard():
    ...
    with tab1:
        ...
        st.plotly_chart(px.pie(pie_counts, values='Count', names='Segment', hole=0.4, color='Segment', color_discrete_map=color_map), width="stretch")
    with tab2:
        st.plotly_chart(px.box(df, x="category", y="avg_grade", color="category"), width="stretch")
...
def page_q1():
    ...
    fig = px.bar(group_att, x='attendance_rate', y='group_name', orientation='h', color='attendance_rate', color_continuous_scale="RdYlGn")
...
def page_q2():
    ...
    st.plotly_chart(px.box(clean_grades, x='type', y='score', color='type'), width="stretch")
...
def page_q3():
    ...
    st.plotly_chart(px.box(df, x='course_name', y='avg_grade', color='course_name', category_orders={'course_name': course_order}), width="stretch")
...
def page_q4():
    ...
    fig = px.scatter(df, x='attendance_rate', y='avg_grade', opacity=0.7)
...
def page_q6():
    ...
    fig = px.bar(top_failed, x='concept_name', y='failure_rate', color='course_id', text_auto='.1%')
...
def page_q7():
    ...
    st.plotly_chart(px.line(mastery_trend, x='assessment_id', y='avg_score', markers=True), width="stretch")
...
def page_q8():
    ...
    st.plotly_chart(px.box(assign_grades, x='is_late', y='score', color='is_late'), width="stretch")
...
def page_q9():
    ...
    fig = px.line(weekly_att, x='session_datetime', y='is_present', markers=True)
    fig.update_layout(yaxis_tickformat='.0%')
    st.plotly_chart(fig, width="stretch")
...
def page_q10():
    ...
    st.plotly_chart(px.bar(age_stats, x='age_band', y='avg_grade', color='attendance_rate', text_auto='.1f'), width="stretch")
...
def page_q11():
    ...
    st.plotly_chart(px.bar(cluster_melted, x='cluster_name', y='Average Value', color='Metric', barmode='group'), width="stretch")
...
def page_q12():
    ...
    st.plotly_chart(px.bar(q12_melted, x='group_id', y='Headcount', color='Metric', barmode='group'), width="stretch")
...
def page_q14():
    ...
    fig = px.bar(top_10, x='risk_score', y='full_name', orientation='h', color='risk_score', hover_data=['group_name', 'failed_concepts'])
    st.plotly_chart(fig, width="stretch")
